...
const okex = new Witnet.Source("https://www.okex.com/api/index/v3/OMG-USDT/constituents")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
...
const mexc = new Witnet.Source("https://www.mxc.com/open/api/v2/market/ticker?symbol=OMG_USDT")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
...
// Retrieves USDT price of OMG from the BKEX API
const bkex = new Witnet.Source("https://api.bkex.cc/v2/q/ticker/price?symbol=OMG_USDT")
  .parseJSONMap() // Parse a `Map` from the retrieved `String`
  .getArray("data") // Access to the `Array` object at `data` key
  .getMap(0) // Access to the `Map` object at index 0
